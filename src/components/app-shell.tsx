...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { data, identity, month, setMonth } = useFinance();
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><span className="demo-dot" /> Minha conta<LogoutButton className="nav-link" /></div>
    </aside>
    <div className="workspace">
      <header className="topbar"><div className="desktop-breadcrumb">Meu espaço <span>/</span> <strong>{navigation.find((item) => item.href === pathname)?.label ?? "Novo lançamento"}</strong></div><Link href="/dashboard" className="mobile-brand"><Brand /></Link><div className="topbar-actions"><ThemeToggle /><label className="month-control"><Icon name="calendar" size={17} /><span className="sr-only">Mês de referência</span><input type="month" aria-label="Mês de referência" value={month} min="2000-01" max="2100-12" onChange={(event) => { if (/^\d{4}-\d{2}$/.test(event.target.value) && event.target.validity.valid) setMonth(event.target.value); }} /></label><Link className="avatar" href="/configuracoes" aria-label="Editar perfil">{(data?.profile.name || identity.name).slice(0, 2).toUpperCase()}</Link></div></header>
      <main id="main-content" className="main-content">{children}<footer className="page-footer">App Finanças <span>Feito para uma vida financeira mais leve.</span><span className="demo-badge">Mês calendário</span></footer></main>
    </div>
