...
export function TransactionForm({ transaction, onClose }: { transaction?: Transaction; onClose?: () => void }) {
  const [saved, setSaved] = useState<{ amount: number; installment: boolean } | null>(null);
  if (saved !== null) return <section className="panel success-panel" role="status"><span className="success-icon"><Icon name="check" size={30} /></span><h2>{saved.installment ? "Compra parcelada registrada!" : "Lançamento registrado!"}</h2><p>{currency(saved.amount)} {saved.installment ? "distribuídos em parcelas previstas." : "salvos nas suas movimentações."}</p><div className="form-actions"><Link href="/historico" className="button primary">Ver movimentações</Link><button className="button secondary" onClick={() => setSaved(null)}>Adicionar outro</button></div></section>;
  return <FinanceGate><TransactionEditor transaction={transaction} onClose={onClose} onSaved={(amount, installment = false) => setSaved({ amount, installment })} /></FinanceGate>;
}
function TransactionEditor({ transaction, onClose, onSaved }: { transaction?: Transaction; onClose?: () => void; onSaved: (amount: number, installment?: boolean) => void }) {
  ...
  if (!data) return null;
  if (installment && !transaction) return <section className="panel form-panel"><InstallmentForm onCancel={() => setInstallment(false)} onSaved={(total, firstDue) => { onSaved(total, true); setMonth(firstDue.slice(0, 7)); }} /></section>;
  ...
  return <form className="panel form-panel" onSubmit={submit} onChange={() => { if (!transaction && !action.pending) id.current = null; }} aria-busy={action.pending}>
    ...
    {!transaction && type === "expense" && <label className="installment-toggle"><input type="checkbox" checked={false} onChange={() => setInstallment(true)} />Compra parcelada</label>}
